...
func clampAABB(aabb world.AABB) (aabb2 world.AABB, ux, uy, width, height uint) {
	p := aabb.Vec2f.Mul(1.0 / Scale).Sub(world.Vec2f{X: 1.0, Y: 1.0}).Floor()
	...
	s := world.Vec2f{X: aabb.Width, Y: aabb.Height}.Mul(1.0 / Scale).Add(world.Vec2f{X: 2.0, Y: 2.0}).Ceil()
	ux = uint(x + Size/2)
	uy = uint(y + Size/2)
...
func (t *Terrain) Collides(entity *world.Entity, seconds float32) bool {
	...
	dx := min(Scale*2/3, dimensions.X*0.499)
	dy := min(Scale*2/3, dimensions.Y*0.499)
