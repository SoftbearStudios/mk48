...
<div class='container'>
	<Section name={entityData[type].label}>
		...
		{:else if entityData[type].subtype === 'submarine'}
			<div class='button' class:selected={altitudeTarget === 0} on:click={toggleAltitudeTarget} title={altitudeTarget === 0 ? 'You are forced to remain surfaced until you reload all fired weapons' : 'You must surface to reload your weapons'}>Surface</div>
		{/if}
	</Section>
</div>
