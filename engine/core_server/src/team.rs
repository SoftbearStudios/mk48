...
impl Repo {
    /// Returns true if the team has space remaining, according to the rules.
    pub fn team_full(
        ...
        member_count as u32 >= rules.team_size_max
    }
    ...
    pub fn accept_player(
        ...
        if let Some(arena) = Arena::get_mut(&mut self.arenas, arena_id) {
            ...
            if let Some(team_id) = captain_team_id {
                if Self::team_full(&arena.sessions, &arena.rules, team_id) {
                    return false;
                }
