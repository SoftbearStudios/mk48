...
use crate::player::TempPlayer;
use atomic_refcell::AtomicRefMut;
use common::protocol::TeamDto;
use common::protocol::{TeamRequest, TeamUpdate};
use kodiak_server::rand::{thread_rng, Rng};
use kodiak_server::{diff_small_n, unwrap_or_return, ArenaService, PlayerId, TeamId, TeamName};
...
fn team_members_max(players: u16) -> usize {
    match players {
        100..=u16::MAX => 8,
        80..=99 => 7,
        60..=79 => 6,
...
pub struct TeamData<G: ArenaService> {
    pub name: TeamName,
...
impl<G: ArenaService> TeamData<G> {
    /// Creates a new team.
    ...
    pub fn is_full(&self, players_online: u16) -> bool {
        self.members.len() >= team_members_max(players_online)
    }

    /// Returns if the team has the maximum possible amount of joiners a.k.a. requests.
    pub fn is_closed(&self) -> bool {
        self.joiners.len() >= TEAM_JOINERS_MAX
    }
...
pub struct PlayerTeamData {
    ...
    /// The last [`TeamId`] the game service was informed of via [`ArenaService::player_changed_team`].
...
impl PlayerTeamData {
    ...
    pub fn forget_state(&mut self) {
        ...
        self.previous_joiners.clear();
        self.previous_joins.clear();
    }
}

/// Part of [`Context`] relating to teams.
pub struct TeamRepo<G: ArenaService> {
    teams: HashMap<TeamId, TeamData<G>>,
    previous: Arc<[TeamDto]>,
}

impl<G: ArenaService> Default for TeamRepo<G> {
    fn default() -> Self {
        Self {
            teams: HashMap::new(),
...
impl<G: ArenaService> TeamRepo<G> {
    ...
    fn assign_team_and_cancel_joins(
        &mut self,
        mut formerly_solo_player: AtomicRefMut<TempPlayer>,
    ...
    fn create_team(
        ...
        if team_members_max(players.real_players_live) == 0 {
        ...
        let team_id = loop {
            let team_id = TeamId(thread_rng().gen());
    ...
    pub(crate) fn request_join(
        ...
        } else if joins.len() >= TEAM_JOINS_MAX {
            // Make room in joins.
            joins.pop_front()
        } else {
            None
    ...
    fn compute_team_dtos(&self, players: &PlayerTupleRepo) -> Vec<TeamDto> {
        self.teams
            .iter()
    ...
    pub(crate) fn player_delta(
        ...
        let (members, joiners, joins) = match &team.status {
    ...
    /// than that.
    pub(crate) fn cleanup_player(&mut self, player_id: PlayerId, players: &PlayerTupleRepo) {
...
mod test {
    ...
    use kodiak_server::rand::prelude::IteratorRandom;
    use kodiak_server::rand::{thread_rng, Rng};
    ...
    #[test]
    fn team_repo() {
        let mut players = PlayerTupleRepo::<MockGame>::default();
