...
impl Translation for LanguageId {
    ...
    fn team_request_hint(self) -> &'static str {
        match self {
            ...
            Arabic => "طلب الانضمام",
            Japanese => "参加をリクエストする",
            Russian => "Запрос на присоединение",
    ...
    fn upgrade_to_label(self, upgrade: &str) -> String {
        match self {
            ...
            Russian => format!("Прокачать до {upgrade}"),
    ...
    fn downgrade_to_label(self, downgrade: &str) -> String {
        match self {
            ...
            Russian => format!("Вернуться к {downgrade}"),
