...
Write-Host "Installing mk48.io..."
$currentDir = Get-Location
$msixPath = ($currentDir.Path + "\" + "mk48.io.sideload.msix")
...
if ($installProc.ExitCode -eq 0) {
	$app = Get-StartApps "mk48.io"

	# If it's an array, then we found multiple matching apps. Grab the last one.
	if ($app -is [array]) {
		Write-Host "Warning: found multiple apps installed named mk48.io. Launching best guess. If the wrong app launches, find the right one in your start menu."
		$app = $app[-1];
	}

	if ($app) {
		Write-Host "Launching mk48.io..."
